# TODO: Translation updated at 2017-05-02 20:02

# game/remy4.rpy:31
translate frenchtltl remy4_42bcad01:

    # c "(Oh, that must be him.)"
    c "(Oh, ça doit être lui.)"

# game/remy4.rpy:57
translate frenchtltl _call_skiptut_28_44e0ddc6:

    # s "My records indicate you have already experienced this scene in a satisfactory manner. Would you like to skip ahead a bit?"
    s "Mes données indiquent que vous avez déjà vécu cette séquence d'une manière satisfaisante. Voulez-vous sauter directement à la sélection des scènes ?"

# game/remy4.rpy:65
translate frenchtltl _call_skiptut_28_690da0b1:

    # s "It looks like you've seen this before. Skip ahead a bit?"
    s "On dirait que vous avez déjà vu cela auparavant. Sauter à la sélection des scènes ?"

# game/remy4.rpy:73
translate frenchtltl _call_skiptut_28_889f24a0:

    # s "So, it turns out you've seen this before. Either you could watch this again, or we could save some time and just skip ahead a bit."
    s "Il semblerait que vous ayez déjà vu cela auparavant. Vous pouvez le revoir, ou alors on pourrait gagner un peu de temps et sauter directement à la sélection des scènes."

# game/remy4.rpy:87
translate frenchtltl _call_skiptut_28_0054b77b:

    # s "As you wish.{cps=2}..{/cps}{w=1.0}{nw}"
    s "Comme vous voulez.{cps=2}..{/cps}{w=1.0}{nw}"

# game/remy4.rpy:107
translate frenchtltl _call_skipcheck_28_0054b77b:

    # s "As you wish.{cps=2}..{/cps}{w=1.0}{nw}"
    s "Comme vous voulez.{cps=2}..{/cps}{w=1.0}{nw}"

# game/remy4.rpy:113
translate frenchtltl _call_skipcheck_28_241427e0:

    # c "Remy, what's that on your face?"
    c "Rémi, qu'est-ce que c'est que ce truc sur ton visage ?"

# game/remy4.rpy:115
translate frenchtltl _call_skipcheck_28_314084ce:

    # Ry smile c "Don't you know what it is?"
    Ry smile c "Tu ne sais pas ce que c'est ?"

# game/remy4.rpy:117
translate frenchtltl _call_skipcheck_28_d293e1c0:

    # c "Well, it looks like lipstick, but I didn't know that was a thing here."
    c "Eh bien, ça ressemble à du rouge à lèvres, mais je ne savais pas que ça pouvait se faire ici."

# game/remy4.rpy:119
translate frenchtltl _call_skipcheck_28_a574db8c:

    # Ry normal c "You're right, it is lipstick. It's not something that existed in this world before, though."
    Ry normal c "Tu as raison, c'est du rouge à lèvres. En revanche, ce n'est pas quelque chose qui existait dans ce monde auparavant."

# game/remy4.rpy:121
translate frenchtltl _call_skipcheck_28_65bedffc:

    # c "How did you get some, then?"
    c "Comment t'en es-tu procuré, alors ?"

# game/remy4.rpy:123
translate frenchtltl _call_skipcheck_28_43dc67d1:

    # Ry smile c "I have one of your PDAs, remember? I studied it a bit and came across the entry on lipstick. Based on the necessary ingredients, I realized I could make some myself, and so I did."
    Ry smile c "J'ai un de vos PDAs, tu te souviens ? Je l'ai étudié un peu et suis tombé sur la rubrique du rouge à lèvre. En me fondant sur les ingrédients nécessaires, j'ai réalisé que je pouvais en fabriquer moi-même, et c'est ce que j'ai fait."

# game/remy4.rpy:125
translate frenchtltl _call_skipcheck_28_8c486a3c:

    # c "Did you do this to look pretty for me?"
    c "As-tu fait ça pour te faire beau pour moi ?"

# game/remy4.rpy:127
translate frenchtltl _call_skipcheck_28_16c204b2:

    # Ry normal c "I just figured you’d miss these kinds of things – things that are normal for you."
    Ry normal c "Je me suis juste dit que des choses comme ça te manqueraient - des choses qui sont normales pour toi."

# game/remy4.rpy:129
translate frenchtltl _call_skipcheck_28_e40f5465:

    # c "It hasn't really been normal back home in quite a while. Lipstick isn't common these days."
    c "Les choses n'ont pas été normales depuis un moment chez moi. Le rouge à lèvres n'est pas commun ces temps-ci."

# game/remy4.rpy:131
translate frenchtltl _call_skipcheck_28_9a58be3d:

    # c "Didn't the entry specify that it's typically used by females?"
    c "La rubrique n'a-t-elle pas spécifié que c'est généralement utilisé par les personnes de sexe féminin ?"

# game/remy4.rpy:133
translate frenchtltl _call_skipcheck_28_10ca1927:

    # Ry "Now that you mention it, I did get that impression, but I found the sentiment a bit puzzling. Why would a product like this only be used by one gender?"
    Ry "Maintenant que tu le mentionnes, j'ai eu cette impression, mais j'ai trouvé cet état d'esprit un peu déroutant. Pourquoi un produit comme celui ci ne serait utilisé que par seulement un genre ?"

# game/remy4.rpy:135
translate frenchtltl _call_skipcheck_28_8ef1e24d:

    # c "Well, I guess social norms dictated that certain traits are only desirable in one gender, thus highlighting them like this would increase the attractiveness."
    c "Eh bien, je crois que les normes sociales nous ont dictées que certains traits ne sont désirables que sur un sexe, les mettant ainsi en valeur comme pour augmenter leur attractivité."

# game/remy4.rpy:137
translate frenchtltl _call_skipcheck_28_0f897892:

    # Ry "Oh, I think I get it now. It's all based on sexual dimorphism. The biological differences between the sexes are much greater in your species than it is in ours, so I suppose the traits you are looking for can be different, too."
    Ry "Oh, Je crois comprendre. Le tout est basé sur le dimorphisme sexuel. Les différences biologiques entre les sexes sont plus importantes chez votre espèce que chez la nôtre, donc je suppose que les caractères que vous recherchez peuvent être aussi différents."

# game/remy4.rpy:139
translate frenchtltl _call_skipcheck_28_19132fb9:

    # Ry "Generally speaking, sexual dimorphism is much greater in mammals than it is in reptiles."
    Ry "De façon générale, le dimorphisme sexuel est bien plus grand chez les mammifères que chez les reptiles."

# game/remy4.rpy:141
translate frenchtltl _call_skipcheck_28_0d6c8006:

    # Ry "While it can vary wildly by species, external indicators are pretty much always present. Like mammaries, for example - an always-present and often obvious reminder."
    Ry "Alors que ça peut varier grandement en fonction des espèces, les indicateurs externes sont presques généralement toujours présents. Comme les glandes mammaires par exemple - un rappel toujours présent et souvent évident."

# game/remy4.rpy:143
translate frenchtltl _call_skipcheck_28_d0432053:

    # Ry shy c "This makes so much more sense now. It's kind of embarrassing."
    Ry shy c "Ça fait beaucoup plus de sens maintenant. C'est assez embarrassant."

# game/remy4.rpy:145
translate frenchtltl _call_skipcheck_28_60d3b085:

    # Ry "Should I... get rid of it?"
    Ry "Devrais-je... m'en débarrasser ?"

# game/remy4.rpy:151
translate frenchtltl _call_skipcheck_28_7c354517:

    # c "Yes. It doesn't really look nice on you."
    c "Oui. Ça ne te va pas vraiment."

# game/remy4.rpy:153
translate frenchtltl _call_skipcheck_28_e92bfd6d:

    # Ry "Oh, sorry."
    Ry "Oh, désolé."

# game/remy4.rpy:165
translate frenchtltl _call_skipcheck_28_9ce4f924:

    # Ry smile "Here, that should do it."
    Ry smile "Voilà, ça devrait faire l'affaire."

# game/remy4.rpy:173
translate frenchtltl _call_skipcheck_28_c82c90d8:

    # c "No, leave it on. It's not as if people here would know about its history."
    c "Non, garde-le. Ce n'est pas comme si les gens ici étaient au courant de son histoire."

# game/remy4.rpy:175
translate frenchtltl _call_skipcheck_28_7543bdcd:

    # Ry normal c "I suppose that's true."
    Ry normal c "Je suppose que c'est vrai."

# game/remy4.rpy:177
translate frenchtltl _call_skipcheck_28_d3e23c96:

    # c "Besides, the thing about females wearing it isn't really a hard rule, and there are males who do it too. Anyone can wear it if they want to, really."
    c "De plus, le fait que ce soit porté essentiellement par des femmes n'est pas vraiment une règle stricte, et il y a des hommes qui le font aussi. N'importe qui peut en porter s'il en a envie, vraiment."

# game/remy4.rpy:179
translate frenchtltl _call_skipcheck_28_d61af9a6:

    # Ry smile c "Well, if you say so."
    Ry smile c "Eh bien, si tu le dis."

# game/remy4.rpy:181
translate frenchtltl _call_skipcheck_28_e8458844:

    # Ry normal c "Still, it's kind of uncomfortable, so I think I'd rather take it off."
    Ry normal c "Il n'empêche que c'est assez inconfortable, Donc je pense que je ferais mieux de l'enlever."

# game/remy4.rpy:183
translate frenchtltl _call_skipcheck_28_a13f1833:

    # c "Uncomfortable?"
    c "Inconfortable ?"

# game/remy4.rpy:185
translate frenchtltl _call_skipcheck_28_c1546f70:

    # Ry "I suppose it's not really suited for our scales."
    Ry "Je suppose que ce n'est pas vraiment adapté pour les écailles."

# game/remy4.rpy:197
translate frenchtltl _call_skipcheck_28_9ce4f924_1:

    # Ry smile "Here, that should do it."
    Ry smile "Voilà, ça devrait faire l'affaire."

# game/remy4.rpy:203
translate frenchtltl _call_skipcheck_28_eecff543:

    # c "Shall we go, then?"
    c "On y va, alors ?"

# game/remy4.rpy:205
translate frenchtltl _call_skipcheck_28_7a9d2314:

    # Ry normal "Actually, it seems we're pretty early. They're probably still setting up."
    Ry normal "En fait, il semblerait qu'il soit assez tôt. Ils seront probablement encore en train de tout monter."

# game/remy4.rpy:207
translate frenchtltl _call_skipcheck_28_9e8264c2:

    # c "I see. Well, we could just stay here in the meantime."
    c "Je vois. Eh bien, on pourrait juste rester ici en attendant."

# game/remy4.rpy:209
translate frenchtltl _call_skipcheck_28_f4be9341:

    # Ry smile "Oh, for sure."
    Ry smile "Oh, bien sûr."

# game/remy4.rpy:217
translate frenchtltl _call_skipcheck_28_e9dca44f:

    # Ry "I just realized I haven't actually been in here since I prepared the apartment for you."
    Ry "Je viens de réaliser que je n'avais pas mis les pattes ici depuis la fois où j'ai préparé l'appartement pour toi."

# game/remy4.rpy:219
translate frenchtltl _call_skipcheck_28_a5783583:

    # Ry "Have you read any of the books I got you?"
    Ry "As-tu lu un des livres que j'ai pris pour toi ?"

# game/remy4.rpy:221
translate frenchtltl _call_skipcheck_28_1abd23c9:

    # c "Oh, so you're the one who stocked the bookshelf?"
    c "Oh, donc tu es celui qui a rempli l'étagère ?"

# game/remy4.rpy:223
translate frenchtltl _call_skipcheck_28_2411d834:

    # Ry "They're all library books. I wanted to provide you with a good sample of what we have to offer, so there should be something in there for anyone."
    Ry "Ce sont tous des livres de la bibliothèque. Je voulais te fournir un bon échantillon de ce qu'on avait à proposer, donc il devrait y en avoir pour tout les goûts."

# game/remy4.rpy:225
translate frenchtltl _call_skipcheck_28_086bcd3a:

    # Ry "Looks like you've read a few books in the Sheridan series."
    Ry "Il semblerait que tu aies lu quelques livres de la saga de Sheridan."

# game/remy4.rpy:231
translate frenchtltl _call_skipcheck_28_d48e8127:

    # Ry "Oh, really?"
    Ry "Oh, vraiment ?"

# game/remy4.rpy:233
translate frenchtltl _call_skipcheck_28_fbe51169:

    # c "I know they're not exactly high-brow, but they're entertaining. And what more do you want a book to be?"
    c "Je sais que ce n'est pas vraiment de la haute littérature, mais ils sont divertissants. Et quoi attendre de plus pour un livre ?"

# game/remy4.rpy:235
translate frenchtltl _call_skipcheck_28_f3665a2c:

    # Ry smile "I guess we all have our own tastes."
    Ry smile "Je suppose que tout est une histoire de goût."

# game/remy4.rpy:239
translate frenchtltl _call_skipcheck_28_ad30e17a:

    # c "Usually, I wouldn't read something like this, but sometimes I'm just not in the mood for anything more complex."
    c "D'ordinaire, je ne lirais pas quelque chose comme ça, mais parfois je ne suis juste pas d'humeur pour quoi que ce soit de plus complexe."

# game/remy4.rpy:241
translate frenchtltl _call_skipcheck_28_ba351dfe:

    # Ry smile "Oh, I understand. Everyone has their guilty pleasures."
    Ry smile "Oh, je comprends. Tout le monde a ses plaisirs coupables."

# game/remy4.rpy:245
translate frenchtltl _call_skipcheck_28_9292a995:

    # c "Can I offer you some food or drink? I've barely used any of the stuff in the kitchen."
    c "Puis-je t'offrir quelque chose à manger ou à boire ? J'ai à peine touché à la nourriture dans la cuisine."

# game/remy4.rpy:247
translate frenchtltl _call_skipcheck_28_abed64d8:

    # Ry normal "Why not?"
    Ry normal "Pourquoi pas ?"

# game/remy4.rpy:249
translate frenchtltl _call_skipcheck_28_39036c48:

    # c "I didn't want to touch anything that didn't at least resemble something I know. After all, I have no idea how to prepare them right or even know if they are safe for me to eat."
    c "Je ne voulais toucher à rien qui ne ressemble à quelque chose que je connaisse. Après tout, je n'ai aucune idée de comment les préparer correctement ou même si je peux les manger."

# game/remy4.rpy:251
translate frenchtltl _call_skipcheck_28_e3950e4a:

    # Ry "Maybe it wasn't such a good idea to get you all those perishables then. I wanted to provide you with a bit of everything, considering I had to prepare the apartment for someone I didn't really know much about."
    Ry "Peut-être que ce n'était pas une si bonne idée de te donner toutes ces denrées périssables. Je voulais te fournir un peu de tout, tout en sachant que je devais préparer l'appartement pour quelqu'un dont je ne savais pas grand-chose."

# game/remy4.rpy:253
translate frenchtltl _call_skipcheck_28_f41caa06:

    # c "Oh, some of them were good. It's interesting to taste the subtle and sometimes not-so-subtle differences in the similar fruits we have back home."
    c "Oh, certains aliments étaient bons. C'était intéressant de voir les différences parfois subtiles, parfois beaucoup moins, avec les fruits similaires que nous avons chez moi."

# game/remy4.rpy:255
translate frenchtltl _call_skipcheck_28_2ba07424:

    # Ry "Did you throw anything out?"
    Ry "En as-tu jeté?"

# game/remy4.rpy:257
translate frenchtltl _call_skipcheck_28_b967ca81:

    # c "Not really. Why do you ask?"
    c "Pas vraiment. Pourquoi me demandes-tu ?"

# game/remy4.rpy:259
translate frenchtltl _call_skipcheck_28_3ef81fa4:

    # Ry "You've been here for nearly two weeks now, so even if some of them were safe for you to eat, they probably aren't anymore."
    Ry "Tu as passé presque deux semaines ici, donc même si tu pouvais les manger sans risques, ça ne doit probablement plus être le cas maintenant."

# game/remy4.rpy:261
translate frenchtltl _call_skipcheck_28_4e8c221f:

    # Ry "Let me clean out your closet. We don't want to have any health hazards for you here, after all."
    Ry "Laisse-moi faire le ménage dans ton placard. On ne veut rien qui représente un danger pour ta santé, après tout."

# game/remy4.rpy:263
translate frenchtltl _call_skipcheck_28_17a1e7a8:

    # c "Well, feel free."
    c "Eh bien, n'hésite pas."

# game/remy4.rpy:273
translate frenchtltl _call_skipcheck_28_2b8bd8af:

    # Ry "Just look at this!"
    Ry "Regarde-moi ça !"

# game/remy4.rpy:277
translate frenchtltl _call_skipcheck_28_d4bc49fc:

    # m "He showed me a juicy-looking, vaguely spherical fruit whose bright red color was unlike any other fruit I had ever seen."
    m "Il me montra un fruit d'apparence juteuse, vaguement sphérique et dont la couleur rouge éclatante ne ressemblait à celle d'aucun autre fruit que j'eusse pu voir."

# game/remy4.rpy:279
translate frenchtltl _call_skipcheck_28_c194efe0:

    # c "That's a very bright red. I don't think even our best apples could compare."
    c "C'est un rouge très brillant. Je ne pense pas que nos meilleures pommes puissent rivaliser."

# game/remy4.rpy:281
translate frenchtltl _call_skipcheck_28_ad116aa1:

    # Ry "Well, they're usually blue. Red means you're supposed to throw them away, as they have expired by then and toxins might have already set in."
    Ry "Eh bien, ils sont bleus normalement. La couleur rouge signifie que tu es supposé les jeter, car ils sont périmés depuis un moment, et ça doit être bourré de toxines."

# game/remy4.rpy:283
translate frenchtltl _call_skipcheck_28_dd6a4d1f:

    # c "Oh. Well, I wasn't planning on eating it anyway."
    c "Oh. Eh bien, je n'avais pas prévu de les manger de toute façon."

# game/remy4.rpy:285
translate frenchtltl _call_skipcheck_28_e6083d51:

    # Ry "Or this one, it's all moldy!"
    Ry "Ou celle-là, il y a de la moisissure partout."

# game/remy4.rpy:287
translate frenchtltl _call_skipcheck_28_1416267a:

    # c "I don't see any mold."
    c "Je ne vois pas de moisissure."

# game/remy4.rpy:289
translate frenchtltl _call_skipcheck_28_b51801e0:

    # Ry "Don't you see these spots?"
    Ry "Tu ne vois pas ces taches ?"

# game/remy4.rpy:291
translate frenchtltl _call_skipcheck_28_bc7573b6:

    # c "Sure, but if that's supposed to be mold, it looks very different from the mold we have back home."
    c "Si, mais si c'est supposé être de la moisissure, elle est très différente de celle qu'on a chez nous."

# game/remy4.rpy:293
translate frenchtltl _call_skipcheck_28_07d1113d:

    # Ry "I see."
    Ry "Je vois."

# game/remy4.rpy:295
translate frenchtltl _call_skipcheck_28_db232029:

    # Ry shy "Maybe it wasn't such a good idea to stock up on perishables. I didn't consider all the implications of doing something like this. Had you eaten the wrong thing, you could be in a hospital right now."
    Ry shy "Peut-être que ce n'était pas une bonne idée de faire le plein de denrées périssables. Je n'ai pas pris en compte tout ce que pouvais impliquer une telle chose. Si tu avais mangé le mauvais aliment, tu aurais pu finir à l'hôpital."

# game/remy4.rpy:297
translate frenchtltl _call_skipcheck_28_8032ab4e:

    # c "Don't stress yourself out about it. Honestly, someone would have to be pretty reckless to just go ahead and start munching on things they don't know about."
    c "Ne te mets pas la pression pour ça. Honnêtement, il aurait fallu être assez téméraire pour juste foncer et commencer à mâcher quelque chose dont on ignore tout."

# game/remy4.rpy:299
translate frenchtltl _call_skipcheck_28_980f08aa:

    # c "If anyone was smart enough to be sent here, they probably wouldn't take a risk like that."
    c "N'importe qui d'assez intelligent pour être envoyé ici ne prendrait probablement pas un tel risque."

# game/remy4.rpy:301
translate frenchtltl _call_skipcheck_28_71fb267a:

    # Ry smile "Well, I'm glad to hear it."
    Ry smile "Eh bien, je suis heureux de l'entendre."

# game/remy4.rpy:303
translate frenchtltl _call_skipcheck_28_ada2cc13:

    # Ry normal "Nevertheless, I better get rid of everything that's past its date. We don't really want the mold in the air, either."
    Ry normal "Néanmoins, je ferais mieux de me débarrasser de tout ce qui a dépassé la date limite. On veut pas vraiment de moisissure dans l'air, non plus."

# game/remy4.rpy:305
translate frenchtltl _call_skipcheck_28_8fc66374:

    # c "If you say so."
    c "C'est toi le patron."

# game/remy4.rpy:309
translate frenchtltl _call_skipcheck_28_ad95221e:

    # m "Once more, he dove into the closet. I heard him rummaging around as he checked the fruits and vegetables thoroughly."
    m "Une fois de plus, il s'enfonça dans le placard. Je l'entendais farfouiller tandis qu'il vérifiait soigneusement les fruits et les légumes."

# game/remy4.rpy:317
translate frenchtltl _call_skipcheck_28_02991e21:

    # Ry "That should be everything."
    Ry "Ca devrait être tout."

# game/remy4.rpy:319
translate frenchtltl _call_skipcheck_28_dd20ffbf:

    # c "Thanks."
    c "Merci."

# game/remy4.rpy:321
translate frenchtltl _call_skipcheck_28_e51965d9:

    # c "I bet the wine didn't go bad, though. Do you want some?"
    c "Je parie que la bouteille de vin n'a pas tourné, elle. Tu en veux un peu ?"

# game/remy4.rpy:323
translate frenchtltl _call_skipcheck_28_e1b54634:

    # Ry normal "I'm not really a big fan of wine."
    Ry normal "Je ne suis pas vraiment un grand fan de vin."

# game/remy4.rpy:325
translate frenchtltl _call_skipcheck_28_dab698e2:

    # c "How come?"
    c "Comment se fait-ce ?"

# game/remy4.rpy:327
translate frenchtltl _call_skipcheck_28_d13d39f4:

    # Ry "I just can't enjoy it as some others do, I suppose. I'm willing to try anything at least once, though."
    Ry "Je n'arrive juste pas à l'apprécier autant que d'autres, je suppose. Par contre, je veux tout essayer au moins une fois."

# game/remy4.rpy:329
translate frenchtltl _call_skipcheck_28_402de06e:

    # c "Anything?"
    c "Tout ?"

# game/remy4.rpy:331
translate frenchtltl _call_skipcheck_28_dc2b5efc:

    # Ry "Well, at least as far as food and drinks are concerned."
    Ry "Eh bien, au moins tout ce qui implique nourriture et boissons."

# game/remy4.rpy:342
translate frenchtltl _call_skipcheck_28_3ae0a6cf:

    # Ry smile "Well, if you want to."
    Ry smile "Eh bien, si tu veux."

# game/remy4.rpy:344
translate frenchtltl _call_skipcheck_28_c48dc32f:

    # c "Sure."
    c "Bien sûr."

# game/remy4.rpy:346
translate frenchtltl _call_skipcheck_28_3454dcb3:

    # c "What kind of wine is this, anyway?"
    c "Quel type de vin est-ce, au passage ?"

# game/remy4.rpy:348
translate frenchtltl _call_skipcheck_28_a6bc6de7:

    # Ry normal "I'm not sure, I just grabbed it when I got all the other stuff."
    Ry normal "Je n'en sais rien, j'ai juste pris cette bouteille quand je faisais les courses."

# game/remy4.rpy:350
translate frenchtltl _call_skipcheck_28_cad3244a:

    # c "Don't tell me you just went for the cheapest one."
    c "Ne me dis pas que tu t'es rabattu sur la bouteille la moins chère."

# game/remy4.rpy:352
translate frenchtltl _call_skipcheck_28_99b4bbb5:

    # Ry smile "Of course not. Everyone knows that you don't just buy the cheapest stuff. You go for the second cheapest."
    Ry smile "Bien sûr que non. Tout le monde sait qu'il ne faut pas acheter la moins chère. Il faut prendre la deuxième moins chère."

# game/remy4.rpy:354
translate frenchtltl _call_skipcheck_28_f047fad9:

    # c "Oh, well."
    c "Ben voyons."

# game/remy4.rpy:356
translate frenchtltl _call_skipcheck_28_7f5b0a66:

    # c "What kind of... container should I get for you, anyway?"
    c "Quel type de... contenant devrai-je aller te chercher ?"

# game/remy4.rpy:358
translate frenchtltl _call_skipcheck_28_3e70cc7d:

    # Ry normal "We're having wine, so let's go for wine glasses. The stem actually makes it possible for me to grip it with my vestigial thumb, so there shouldn't be any problems."
    Ry normal "On va boire du vin, donc partons sur des verres à vin. Le pied me permet de le tenir avec mon pouce vestigial, donc il ne devrait y avoir aucun problème."

# game/remy4.rpy:360
translate frenchtltl _call_skipcheck_28_86148265:

    # c "Alright."
    c "Ça roule."

# game/remy4.rpy:366
translate frenchtltl _call_skipcheck_28_7cdc3eb6:

    # c "Here you go."
    c "Voilà pour toi."

# game/remy4.rpy:368
translate frenchtltl _call_skipcheck_28_5bfcfb4d:

    # Ry "What shall we drink to?"
    Ry "A quoi devrions-nous trinquer ?"

# game/remy4.rpy:376
translate frenchtltl _call_skipcheck_28_1ffd676a:

    # Ry smile "Alright. To an interesting afternoon and whatever it may bring."
    Ry smile "D'accord. A un après midi bien rempli et quoi que ce soit que ça puisse amener."

# game/remy4.rpy:382
translate frenchtltl _call_skipcheck_28_ed05a844:

    # Ry smile "Alright. To a wonderful friendship."
    Ry smile "D'accord. A une formidable amitié."

# game/remy4.rpy:389
translate frenchtltl _call_skipcheck_28_00974107:

    # Ry smile "Alright. To whatever, then."
    Ry smile "D'accord. Advienne que pourra, alors."

# game/remy4.rpy:399
translate frenchtltl _call_skipcheck_28_2b440c0e:

    # m "I took a sip from my wine glass. Remy really hadn't been kidding about getting the second cheapest bottle, because that was exactly what it tasted like."
    m "Je pris une gorgée de mon verre à vin. Rémi ne plaisantait pas quand au sujet de prendre la deuxième bouteille la moins chère, parce que ça en avait exactement le goût."

# game/remy4.rpy:401
translate frenchtltl _call_skipcheck_28_85881e65:

    # c "How do you like it?"
    c "Qu'est-ce que tu en penses ?"

# game/remy4.rpy:403
translate frenchtltl _call_skipcheck_28_d5c6589c:

    # Ry "Must be an acquired taste, I think."
    Ry "Ca doit être pour les palais raffinés, je pense."

# game/remy4.rpy:405
translate frenchtltl _call_skipcheck_28_a3d65aa3:

    # Ry "How about you?"
    Ry "Et toi ?"

# game/remy4.rpy:411
translate frenchtltl _call_skipcheck_28_8505c624:

    # c "It's okay. I mean, you can't really expect much from a cheap bottle of wine."
    c "C'est correct. Je veux dire, tu ne peux pas vraiment attendre grand chose d'une bouteille de vin bon marché."

# game/remy4.rpy:413
translate frenchtltl _call_skipcheck_28_07d1113d_1:

    # Ry "I see."
    Ry "Je vois."

# game/remy4.rpy:417
translate frenchtltl _call_skipcheck_28_a6fd0a7f:

    # c "It's bad. I'm not surprised that you don't like it either."
    c "C'est mauvais. Je ne suis pas surpris que tu n'aimes pas non plus."

# game/remy4.rpy:419
translate frenchtltl _call_skipcheck_28_ce740940:

    # Ry smile "Maybe I should buy the third cheapest next time."
    Ry smile "Peut-être que je devrais acheter la troisième moins chère la prochaine fois."

# game/remy4.rpy:421
translate frenchtltl _call_skipcheck_28_13251290:

    # c "Maybe."
    c "Peut-être."

# game/remy4.rpy:429
translate frenchtltl _call_skipcheck_28_770cab01:

    # c "I'm not a fan either, so I guess we don't need to waste a good bottle of wine by opening it."
    c "Je ne suis pas fan non plus, donc je crois qu'on a pas besoin de gâcher une bonne bouteille de vin en l'ouvrant."

# game/remy4.rpy:431
translate frenchtltl _call_skipcheck_28_6c582c4a:

    # Ry "I'm not sure if this one would qualify as a \"good bottle\"."
    Ry "Je ne sais pas si celle là pourrait être qualifiée de \"bonne bouteille\"."

# game/remy4.rpy:433
translate frenchtltl _call_skipcheck_28_cad3244a_1:

    # c "Don't tell me you just went for the cheapest one."
    c "Ne me dis pas que tu t'es rabattu sur la bouteille la moins chère."

# game/remy4.rpy:435
translate frenchtltl _call_skipcheck_28_7b26bb2e:

    # Ry smile "Of course not. Everyone knows that you don't buy the cheapest stuff. You go for the second cheapest."
    Ry smile "Bien sûr que non. Tout le monde sait qu'il ne faut pas acheter la moins chère. Il faut prendre la deuxième moins chère."

# game/remy4.rpy:437
translate frenchtltl _call_skipcheck_28_f047fad9_1:

    # c "Oh, well."
    c "Ben voyons..."

# game/remy4.rpy:442
translate frenchtltl _call_skipcheck_28_86b88713:

    # Ry "Just look at the time. I think we could start heading to the festival now."
    Ry "Regarde l'heure. Je pense qu'on devrait commencer à se diriger vers le festival."

# game/remy4.rpy:444
translate frenchtltl _call_skipcheck_28_4f7f32d4:

    # c "Sure, let's go."
    c "Bien sûr, on y va."

# game/remy4.rpy:456
translate frenchtltl _call_skipcheck_28_cd7294e4:

    # Ry "This is strange. Usually there'd be a crowd by now."
    Ry "C'est étrange. Normalement, c'est bondé à cette heure-ci."

# game/remy4.rpy:458
translate frenchtltl _call_skipcheck_28_c11a081c:

    # c "Do you think they canceled it? With Reza still being out there, maybe they thought it wasn't a good idea to have a public spectacle like this."
    c "Tu penses qu'ils l'ont annulé ? Avec Reza toujours en liberté, peut-être qu'ils ont pensé que ce n'était pas une bonne idée d'avoir un événement public comme celui-ci."

# game/remy4.rpy:460
translate frenchtltl _call_skipcheck_28_035ae435:

    # Ry "Cancel the summer festival? Inconceivable!"
    Ry "Annuler le festival d'été ? Inconcevable !"

# game/remy4.rpy:462
translate frenchtltl _call_skipcheck_28_51bec4ab:

    # c "It doesn't really look like a festival is going on right now, though."
    c "En attendant, on ne dirait pas qu'un festival est en train de se dérouler, là."

# game/remy4.rpy:464
translate frenchtltl _call_skipcheck_28_a6f3bbb5:

    # Ry "I don't know what's going on."
    Ry "Je ne sais pas ce qui se trame."

# game/remy4.rpy:466
translate frenchtltl _call_skipcheck_28_14f640c0:

    # c "Hey, I think I know this guy. Let's go and ask him."
    c "Hey, Je crois que je connais cette personne. Allons lui demander."

# game/remy4.rpy:472
translate frenchtltl _call_skipcheck_28_08ae875a:

    # Fv "Hey, [player_name]. How can I help you?"
    Fv "Hey, [player_name]. Comment puis-je t'aider ?"

# game/remy4.rpy:474
translate frenchtltl _call_skipcheck_28_5574c33b:

    # c "We were just planning to go to the festival, but it doesn't seem like there's much going on here yet."
    c "On avait prévu d'aller au festival, mais il semblerait qu'il ne se passe pas grand chose, là, tout de suite."

# game/remy4.rpy:476
translate frenchtltl _call_skipcheck_28_be6f6863:

    # Fv "You came to the right place, but the festival is not starting for another hour, at least."
    Fv "Vous êtes au bon endroit, mais le festival ne commence pas avant une heure, au moins."

# game/remy4.rpy:478
translate frenchtltl _call_skipcheck_28_a91e7ba9:

    # Ry "What? But the festival has always started at this time."
    Ry "Pardon ? Mais le festival a toujours commencé à cette heure-ci."

# game/remy4.rpy:480
translate frenchtltl _call_skipcheck_28_ff11f82d:

    # Fv "You must have missed the memo, then. They moved it by an hour. Something about the daylight and making the fireworks more visible as a result."
    Fv "Dans ce cas, vous avez dû rater le mémo. ils l'ont décalé d'une heure. Quelque chose à propos de la lumière du jour et de rendre les feux d'artifice plus visibles en conséquence."

# game/remy4.rpy:482
translate frenchtltl _call_skipcheck_28_61255d84:

    # Ry "Well, that's a shame."
    Ry "Oh, c'est dommage."

# game/remy4.rpy:484
translate frenchtltl _call_skipcheck_28_0d67c413:

    # c "And I'm kinda getting hungry, too."
    c "Et je commence à avoir faim, aussi."

# game/remy4.rpy:486
translate frenchtltl _call_skipcheck_28_787dbf46:

    # Fv "I can help you with that."
    Fv "Pour ça, je peux vous aider."

# game/remy4.rpy:488
translate frenchtltl _call_skipcheck_28_0e8990e1:

    # c "What are you selling?"
    c "Qu'est-ce que vous vendez ?"

# game/remy4.rpy:490
translate frenchtltl _call_skipcheck_28_4991f593:

    # Fv "Skewers, meat balls. All kinds of grilled things, really."
    Fv "Des brochettes, des boulettes de viande. Des grillades en tout genre."

# game/remy4.rpy:492
translate frenchtltl _call_skipcheck_28_a5dec48f:

    # Ry smile "Well, I could go for some meat balls. How about you?"
    Ry smile "Eh bien, je serais partant pour des boulettes. Et toi ?"

# game/remy4.rpy:514
translate frenchtltl _call_skipcheck_28_833eb8a5:

    # Fv "No problem, just let me pack this up for you."
    Fv "Pas de problème, laissez moi juste emballer ça pour vous."

# game/remy4.rpy:520
translate frenchtltl _call_skipcheck_28_acad0f7e:

    # Ry "I guess this makes us your first customers of the day. What do I owe you?"
    Ry "Il faut croire que ça fait de nous vos premiers clients de la journée. Combien je vous dois ?"

# game/remy4.rpy:522
translate frenchtltl _call_skipcheck_28_b47d1661:

    # Fv "For you two, it's free."
    Fv "Pour vous deux, c'est gratuit."

# game/remy4.rpy:524
translate frenchtltl _call_skipcheck_28_5943f041:

    # Ry "That's very kind of you. Thank you."
    Ry "C'est très gentil à vous. Merci."

# game/remy4.rpy:526
translate frenchtltl _call_skipcheck_28_b9b107ba:

    # Fv "Have fun at the festival."
    Fv "Amusez-vous bien au festival."

# game/remy4.rpy:528
translate frenchtltl _call_skipcheck_28_5b6f67fd:

    # c "Thanks!"
    c "Merci !"

# game/remy4.rpy:536
translate frenchtltl _call_skipcheck_28_252423fc:

    # c "So, what do we do now? Should we go back?"
    c "Bon, qu'est-ce qu'on fait maintenant ? Devrait-on rentrer ?"

# game/remy4.rpy:538
translate frenchtltl _call_skipcheck_28_17352176:

    # Ry "Well, we're already here and got all this stuff. Let's just sit down somewhere."
    Ry "Eh bien, on est déjà là avec de la nourriture. Allons nous asseoir quelque part."

# game/remy4.rpy:540
translate frenchtltl _call_skipcheck_28_c48dc32f_1:

    # c "Sure."
    c "Ca marche."

# game/remy4.rpy:544
translate frenchtltl _call_skipcheck_28_bfc9e371:

    # m "We went off to the side as more dragons arrived and started setting up their various booths."
    m "Nous nous décalâmes sur le côté, tandis que plus de dragons arrivaient et commençaient à monter leurs divers stands."

# game/remy4.rpy:546
translate frenchtltl _call_skipcheck_28_1e7e3bb6:

    # c "How are the meat balls?"
    c "Comment sont les boulettes ?"

# game/remy4.rpy:550
translate frenchtltl _call_skipcheck_28_7e2939f1:

    # Ry "They're actually pretty good. How about your stuff?"
    Ry "Elle sont plutôt bonnes, en fait. Et toi, ce que tu as pris ?"

# game/remy4.rpy:552
translate frenchtltl _call_skipcheck_28_71d2a125:

    # c "It's great. I'm surprised, honestly. You never know with stuff that comes out of food carts."
    c "C'est super bon. Je suis surpris, pour être honnête. On ne sait jamais avec de la nourriture provenant de chariots."

# game/remy4.rpy:554
translate frenchtltl _call_skipcheck_28_35448188:

    # Ry "You're right. Maybe we got lucky."
    Ry "Tu as raison. Peut-être qu'on a été chanceux."

# game/remy4.rpy:556
translate frenchtltl _call_skipcheck_28_4204bf66:

    # c "Or maybe it's because it's still fresh."
    c "Ou peut-être parce que c'est encore frais. "

# game/remy4.rpy:558
translate frenchtltl _call_skipcheck_28_1a2058d7:

    # Ry smile "Who knows."
    Ry smile "Qui sait."

# game/remy4.rpy:560
translate frenchtltl _call_skipcheck_28_7adb5ad2:

    # c "What was that about fireworks, though? Is the big show coming up later?"
    c "C'était quoi cette histoire à propos des feux d'artifice ? Le bouquet final va se dérouler plus tard ?"

# game/remy4.rpy:562
translate frenchtltl _call_skipcheck_28_ac5ed8f7:

    # Ry normal "Oh, no. The festival is just getting started, and there are fireworks every day. But the big show is what everyone wants to see. That one is happening in a few days. It officially concludes the festivities."
    Ry normal "Oh non. le festival ne fait que commencer, et il y a des feux d'artifice tout les jours. Mais le bouquet final est celui que tout le monde veut voir. Celui-ci se déroule dans quelques jours. Il conclut officiellement les festivités."

# game/remy4.rpy:564
translate frenchtltl _call_skipcheck_28_89aefe22:

    # c "I see."
    c "Je vois."

# game/remy4.rpy:566
translate frenchtltl _call_skipcheck_28_6f342349:

    # m "A while later, the festival had kicked off and the crowds were already coming in. Dragons of all shapes and colors filled the paths, ready to see the sights the festival had to offer."
    m "Un peu plus tard, le festival fut ouvert et la foule pénétrait sur les lieux. Des dragons de toutes les formes et les couleurs remplissaient les rues, prêts à voir les curiosités que le festival avait à leur offrir."

# game/remy4.rpy:568
translate frenchtltl _call_skipcheck_28_9d5e9a20:

    # m "Booths lined both sides of the way. It reminded me a lot of a typical carnival setup, with the booths being filled with shops, food stands, games and much more."
    m "Les baraques étaient alignés des deux côtés de la rue. Cela me rappelait beaucoup l'organisation typique d'une fête foraine, avec les stands remplis de boutiques, de quoi se restaurer, de jeux et plus encore."

# game/remy4.rpy:570
translate frenchtltl _call_skipcheck_28_ddfec17d:

    # c "Seems the festival has already started. Let's go."
    c "Il semblerait que le festival a déjà démarré. Allons-y."

# game/remy4.rpy:572
translate frenchtltl _call_skipcheck_28_ab3388b6:

    # Ry "You know what, how about we just stay here? I really don't like crowds."
    Ry "Tu sais quoi, pourquoi ne resterions-nous pas ici ? Je n'aime pas vraiment la foule."

# game/remy4.rpy:574
translate frenchtltl _call_skipcheck_28_7f5fd2f5:

    # Ry "I've seen the festival so many times already, I'd rather just stay here with you."
    Ry "J'ai vu le festival un paquet de fois, je préfèrerais juste rester ici avec toi."

# game/remy4.rpy:576
translate frenchtltl _call_skipcheck_28_744dc376:

    # c "I thought you wanted to show me around, but I suppose we can just stay here if you'd prefer that."
    c "Je pensais que tu voulais me faire faire le tour, mais je suppose qu'on peut juste rester ici si tu préfères ça."

# game/remy4.rpy:578
translate frenchtltl _call_skipcheck_28_5b797ec5:

    # Ry "The main attraction is always the big fireworks show at the end of the festival, and that one is not going to happen today, anyway."
    Ry "L'attraction principale demeure le grand feu d'artifice à la fin du festival, et celui-ci n'est pas pour aujourd'hui, de toute façon."

# game/remy4.rpy:580
translate frenchtltl _call_skipcheck_28_1ebb3b2c:

    # c "So, why'd we need to come here in the first place, then? Is something wrong?"
    c "Donc, pourquoi on aurait eu besoin de venir ici, du coup ? Quelque chose ne va pas ?"

# game/remy4.rpy:582
translate frenchtltl _call_skipcheck_28_30318f06:

    # Ry "I just had to realize that as nice as it is right now, you'll have to leave this world again eventually."
    Ry "J'ai juste été obligé de réaliser que, aussi agréable qu'est l'instant présent, tu devras repartir chez toi finalement."

# game/remy4.rpy:584
translate frenchtltl _call_skipcheck_28_471cb936:

    # Ry shy "I've been alone for such a long time now. I didn't even have a single friend."
    Ry shy "J'ai été seul depuis si longtemps. Je n'avais même pas un seul ami."

# game/remy4.rpy:586
translate frenchtltl _call_skipcheck_28_027b6343:

    # Ry "In my job, I always have to smile when serving the customers. How could I be surrounded by people all the time, yet feel so alone?"
    Ry "A mon boulot, je dois toujours sourire quand je sers les clients. Comment pouvais-je être entouré de tant de personnes tout ce temps, et pourtant me sentir si seul ?"

# game/remy4.rpy:588
translate frenchtltl _call_skipcheck_28_62e74721:

    # Ry normal "All those people have their own lives, families, friends and relationships. Yet I had none of that. In a world where everyone else seemed to be happy, I was the odd one out who had to pretend in order to belong."
    Ry normal "Toutes ces personnes ont leurs propres vies, familles, amis et relations. Et moi, je n'ai rien de tout ça. Dans un monde où tout le monde semblait être heureux, j'étais un cas particulier qui devait faire semblant pour me fondre dans la masse."

# game/remy4.rpy:590
translate frenchtltl _call_skipcheck_28_a3bc31e4:

    # Ry shy "How could that not have eaten away at me, day after day?"
    Ry shy "Comment ça a pu ne pas me bouffer, jour après jour ?"

# game/remy4.rpy:592
translate frenchtltl _call_skipcheck_28_f01fee62:

    # Ry normal "Yet, one day, I heard about the plans of the humans coming here."
    Ry normal "Et puis, un jour, j'ai entendu parler des plans des humains pour venir ici."

# game/remy4.rpy:594
translate frenchtltl _call_skipcheck_28_897f35dd:

    # Ry "Like many others, I was very excited at the prospect of our myths coming true."
    Ry "Comme beaucoup d'autre, j'étais très excité par la perspective que nos mythes deviennent réalité."

# game/remy4.rpy:596
translate frenchtltl _call_skipcheck_28_2f856f93:

    # Ry "And then, the day came when Reza stepped through the portal - amidst all the pomp and fanfare you could imagine. It was quite a sight to behold, really."
    Ry "Et puis, le jour où Reza traversa le portail- au milieu de toute les fastes et fanfares que tu puisses imaginer. C'était une sacrée vue à contempler, vraiment."

# game/remy4.rpy:598
translate frenchtltl _call_skipcheck_28_9a0f8f61:

    # Ry "You wouldn't believe how much joy I felt during that moment. Emera, of course, took all the limelight she could. First there was a big speech in front of a huge crowd, then a procession after she had officially welcomed Reza."
    Ry "Tu n'imagines pas la joie que j'ai ressenti durant ce moment. Emera, bien sûr, attira les projecteurs autant qu'elle le pouvait. D'abord, il y a eu un grand discours devant une grande foule, puis un défilé après qu'elle a officiellement accueilli Reza."

# game/remy4.rpy:600
translate frenchtltl _call_skipcheck_28_2beeb5d5:

    # Ry "When it was time for you to arrive and it was decided that it wouldn't be a public event this time due to security concerns, I knew that Emera wasn't going to be there if she wasn't able to make it all about herself."
    Ry "Quand ce fut ton tour d'arriver et qu'il fut décidé que ça ne serait pas un évènement public cette fois, à cause d'inquiétudes en matière de sécurité, je savais qu'Emera n'allait pas être là si elle ne pouvait pas être au centre de l'attention."

# game/remy4.rpy:602
translate frenchtltl _call_skipcheck_28_b43eb1ee:

    # Ry shy "I begged her to let me do it, just to be able to get this close to a human at least once. To talk with them and see what they were like - to experience them."
    Ry shy "Je l'ai suppliée de me laisser m'en charger, juste pour pouvoir approcher un humain au moins une fois. Pour parler avec lui et voir ce à quoi ils ressemblaient - pour les découvrir."

# game/remy4.rpy:604
translate frenchtltl _call_skipcheck_28_b24f4e0f:

    # Ry "She told me that if I did a few favors for her, I could be the one to greet you."
    Ry "Elle m'a dit que si je lui rendais quelques services, je pouvais être celui qui allait t'accueillir."

# game/remy4.rpy:606
translate frenchtltl _call_skipcheck_28_58369a01:

    # Ry sad "I knew that she wasn't going to be there anyway, but if I didn't do what she wanted, she would just send some other lackey instead."
    Ry sad "Je savais qu'elle n'allait pas être là de toute façon, mais si je ne faisais pas ce qu'elle disait, elle enverrait juste un autre larbin à la place."

# game/remy4.rpy:608
translate frenchtltl _call_skipcheck_28_8b9ae242:

    # Ry "So I did everything she asked for."
    Ry "Donc, j'ai fait tout ce qu'elle m'a demandé."

# game/remy4.rpy:610
translate frenchtltl _call_skipcheck_28_ff716e0f:

    # Ry normal "She did keep her word, though - and in the end, I was the one who would introduce you to this little world of ours."
    Ry normal "En fin de compte, elle a tenu parole - et au final, je fus celui qui allait t'introduire à ce petit monde qu'est le nôtre."

# game/remy4.rpy:612
translate frenchtltl _call_skipcheck_28_a0a9844a:

    # Ry "It might not have seemed like much to you, but with just us two and no one else being there, it was something very special for me."
    Ry "Ca n'a pas dû te sembler être quelque chose de gros, mais avec seulement nous deux et personne d'autre, c'était quelque chose de très spécial pour moi."

# game/remy4.rpy:614
translate frenchtltl _call_skipcheck_28_cc369ecb:

    # Ry "It was not just some public spectacle like the summer festival or Reza's arrival, but for the first time in so long, I felt like that moment was about you and me - and no one else."
    Ry "Ce n'était pas juste un quelconque spectacle public comme le festival d'été ou l'arrivée de Reza, mais pour la première fois depuis si longtemps, je me sentais comme si ce moment était juste entre toi et moi - et personne d'autre."

# game/remy4.rpy:616
translate frenchtltl _call_skipcheck_28_27dc9ac9:

    # Ry "For someone in my position, it just made me very happy to have such an extraordinary thing happen to me."
    Ry "Pour quelqu'un dans ma situation, ça m'a juste rendu très heureux qu'une chose aussi extraordinaire m'arrive à moi."

# game/remy4.rpy:618
translate frenchtltl _call_skipcheck_28_16ac0834:

    # Ry "Of course I was very curious to see how much our myths would line up with reality, and I was even more overjoyed when you visited me in the library."
    Ry "Bien sûr, j'étais vraiment curieux de voir à quel point nos mythes colleraient avec la réalité, et j'étais plus que content quand tu m'as rendu visite à la librairie."

# game/remy4.rpy:620
translate frenchtltl _call_skipcheck_28_5c9f8e07:

    # Ry "When you accepted my invitation and came into my home, I was just glad to have you there."
    Ry "Quand tu as accepté mon invitation et est venu chez moi, j'étais si content de t'avoir à la maison."

# game/remy4.rpy:622
translate frenchtltl _call_skipcheck_28_bec38e42:

    # Ry "It seemed like you wanted to be there and weren't just someone out to use me in some way. I began to feel hope."
    Ry "C'est comme si tu voulais vraiment être là et que tu n'étais pas juste là pour m'utiliser d'une quelconque manière. J'ai commencé à ressentir de l'espoir."

# game/remy4.rpy:624
translate frenchtltl _call_skipcheck_28_0596f680:

    # Ry "When I met you in the park, I think the only reason I could tell you everything about myself was because I knew you were going to leave again eventually."
    Ry "Quand je t'ai retrouvé dans le parc, je pense que la seule raison pour laquelle j'ai été capable de tout te dire à mon sujet, c'est parce que je savais que tu allais repartir éventuellement."

# game/remy4.rpy:626
translate frenchtltl _call_skipcheck_28_bf19654f:

    # Ry "And now, after all of this, I can hardly believe you're still here with me."
    Ry "Et maintenant, après tout ça, je peux à peine croire que tu es toujours là avec moi."

# game/remy4.rpy:628
translate frenchtltl _call_skipcheck_28_e77c5ee5:

    # c "Why wouldn't I be?"
    c "Pourquoi ne le serais-je pas ?"

# game/remy4.rpy:630
translate frenchtltl _call_skipcheck_28_1534bc2b:

    # Ry shy "You're just so nice to me."
    Ry shy "Tu es juste si gentil avec moi."

# game/remy4.rpy:632
translate frenchtltl _call_skipcheck_28_4f312056:

    # Ry "Yet, even after the time I've got to spend with you, it seems that soon, I'll have to let you go again."
    Ry "Et pourtant, même après tout le temps que j'ai passé avec toi, il semblerait que, tôt ou tard, je devrai te laisser partir."

# game/remy4.rpy:636
translate frenchtltl remy4skip1_7d456fef:

    # Ry normal "At least I got to know you."
    Ry normal "Au moins, j'ai eu la chance de te connaître."

# game/remy4.rpy:640
translate frenchtltl remy4skip1_6be34af2:

    # Ry "And thanks to you, I was also able to reconnect with Adine. Now, it just seems so petty to not have spoken with her for such a long time."
    Ry "Et grâce à toi, j'ai été capable de reprendre contact avec Adine. Bon, c'est vrai, ça semble juste mesquin de ne pas lui avoir parlé depuis aussi longtemps."

# game/remy4.rpy:642
translate frenchtltl remy4skip1_9159c31e:

    # Ry "All those times when I felt alone and depressed, I really was only a phone call away from someone who still cared about me."
    Ry "Toutes ces fois où je me suis senti seul et déprimé, J'étais à un coup de téléphone de quelqu'un qui tenait toujours à moi."

# game/remy4.rpy:644
translate frenchtltl remy4skip1_3c3feecb:

    # c "But now that you have, this also means that your worst times are over. You're not alone anymore."
    c "Mais maintenant que tu l'as rappelé, cela veut dire aussi que tes heures les plus sombres sont derrière toi. Tu n'es plus seul désormais."

# game/remy4.rpy:646
translate frenchtltl remy4skip1_d9441013:

    # Ry smile "That is true."
    Ry smile "C'est vrai."

# game/remy4.rpy:648
translate frenchtltl remy4skip1_932b01d8:

    # Ry normal "To some people, it may not matter much if they have one friend more or less. But you know what makes the biggest difference? Having one when you didn't used to."
    Ry normal "Pour certaines personnes, ça ne doit pas importer beaucoup s'ils ont un ami de plus ou de moins. Mais tu sais ce qui fait la plus plus grande différence ? En avoir un quand tu n'en as jamais eu."

# game/remy4.rpy:650
translate frenchtltl remy4skip1_0d5f907c:

    # Ry "We even went to the orphanage together some time ago to help with the kids there. It seems one of them has taken quite a liking to me."
    Ry "Nous sommes même allé à l'orphelinat ensemble il y a peu pour aider avec les enfants là bas. Il semblerait même qu'un des enfants m'apprécie beaucoup."

# game/remy4.rpy:652
translate frenchtltl remy4skip1_eef41f58:

    # c "That sounds lovely."
    c "Ca a l'air génial."

# game/remy4.rpy:654
translate frenchtltl remy4skip1_2adf9517:

    # Ry "Her name is Vara, and she -"
    Ry "Elle s'appelle Vara, et elle..."

# game/remy4.rpy:656
translate frenchtltl remy4skip1_7d275a01:

    # c "Vara? I remember her."
    c "Vara ? Je me rappelle d'elle."

# game/remy4.rpy:658
translate frenchtltl remy4skip1_9d4673c7:

    # Ry "You know about her, then? It's such a sad story."
    Ry "Tu es au courant à son sujet, alors ? C'est une bien triste histoire."

# game/remy4.rpy:663
translate frenchtltl remy4skip1_2c33a8c1:

    # c "It can't be all that bad. Ever since I came here, I've met plenty of lovely people - all with their own relationships, struggles and dreams."
    c "Ca ne peut pas être si catastrophique. Depuis que je arrivé ici, j'ai rencontré plein de personnes adorables - tous avec leurs relations, leurs luttes et leurs rêves."

# game/remy4.rpy:665
translate frenchtltl remy4skip1_dcc66629:

    # c "I know you think you've been alone here all this time, but don't go back into that kind of thinking when I'm gone."
    c "Je sais que tu penses que tu a été seul depuis tout ce temps, mais ne retourne pas dans ce schéma de pensée quand je serai parti."

# game/remy4.rpy:667
translate frenchtltl remy4skip1_edaefd0b:

    # c "Rather, why don't you take it as an opportunity? Now that we're here, who's to say that you won't be able to find others once I'm gone?"
    c "A la place, pourquoi ne prendrais tu pas ça comme une opportunité ? Maintenant que nous en sommes là, qui dit que tu ne seras pas capable de trouver d'autre personnes une fois que je serai parti ?"

# game/remy4.rpy:669
translate frenchtltl remy4skip1_69be09bc:

    # c "I know you're still stuck for a while because of your job, but after that, have you considered a change of scenery?"
    c "Je sais que tu es encore coincé pour un moment à cause de ton boulot, mais après ça, as-tu envisagé de changer de décor ?"

# game/remy4.rpy:671
translate frenchtltl remy4skip1_d4e2ef2d:

    # c "You could move to a different place, with all new people and opportunities. Who knows what you might find elsewhere?"
    c "Tu pourrais déménager autre part, avec tout plein de nouvelles personnes et d'opportunités. Qui sait ce que tu pourrais trouver ailleurs ?"

# game/remy4.rpy:673
translate frenchtltl remy4skip1_d0ab1843:

    # Ry "A completely new start? I like that idea."
    Ry "Un nouveau départ ? J'aime bien l'idée."

# game/remy4.rpy:676
translate frenchtltl remy4skip1_bb0c0326:

    # Ry "But enough about me."
    Ry "Mais assez parlé de moi."

# game/remy4.rpy:696
translate frenchtltl _call_skiptut_29_0ff5f677:

    # s "My records indicate you have already experienced this scene in a satisfactory manner. Would you like to skip to the end?"
    s "Mes données indiquent que vous avez déjà vécu cette séquence d'une manière satisfaisante. Voulez-vous sauter directement à la sélection des scènes ?"

# game/remy4.rpy:704
translate frenchtltl _call_skiptut_29_eee83226:

    # s "It looks like you've seen this before. Skip to the end of this scene?"
    s "On dirait que vous avez déjà vu cela auparavant. Sauter à la sélection des scènes ?"

# game/remy4.rpy:712
translate frenchtltl _call_skiptut_29_e7709aa0:

    # s "So, it turns out you've seen this before. Either you could watch this again, or we could save some time and just skip to the end of this scene."
    s "Il semblerait que vous ayez déjà vu cela auparavant. Vous pouvez le revoir, ou alors on pourrait gagner un peu de temps et sauter directement à la sélection des scènes."

# game/remy4.rpy:726
translate frenchtltl _call_skiptut_29_0054b77b:

    # s "As you wish.{cps=2}..{/cps}{w=1.0}{nw}"
    s "Comme vous voulez.{cps=2}..{/cps}{w=1.0}{nw}"

# game/remy4.rpy:746
translate frenchtltl _call_skipcheck_29_0054b77b:

    # s "As you wish.{cps=2}..{/cps}{w=1.0}{nw}"
    s "Comme vous voulez.{cps=2}..{/cps}{w=1.0}{nw}"

# game/remy4.rpy:750
translate frenchtltl _call_skipcheck_29_efc6b8c8:

    # Ry "What will you do once you get back to your own world again?"
    Ry "Que vas-tu faire une fois rentré dans ton monde ?"

# game/remy4.rpy:752
translate frenchtltl _call_skipcheck_29_defef769:

    # c "I'm not sure. I don't even know what will happen to me when I return."
    c "Je ne sais pas. Je ne sais même pas ce qui m'arrivera quand je rentrerai."

# game/remy4.rpy:754
translate frenchtltl _call_skipcheck_29_941e8df6:

    # c "These last few days, a lot of things have happened that have changed my perspective on what I'm doing here, Reza and those who sent me."
    c "Ces derniers jours, beaucoup de choses sont arrivées, ce qui a changé ma vision des choses sur ma mission ici, Reza, et ceux qui m'ont envoyé."

# game/remy4.rpy:756
translate frenchtltl _call_skipcheck_29_43488afa:

    # Ry "Is that a bad thing?"
    Ry "Est-ce une mauvaise chose ?"

# game/remy4.rpy:758
translate frenchtltl _call_skipcheck_29_1cb1d0f8:

    # c "I don't know yet. At the very least, though, I can say that I've done what I believed was right."
    c "Je ne sais pas encore. Au moins, je peux dire que j'ai fait ce que je pensais être juste."

# game/remy4.rpy:760
translate frenchtltl _call_skipcheck_29_5a9a0466:

    # Ry "You should be careful with rhetoric like that."
    Ry "Tu devrais faire attention avec une rhétorique comme celle là."

# game/remy4.rpy:762
translate frenchtltl _call_skipcheck_29_bb00a52c:

    # c "What do you mean?"
    c "Que veux-tu dire ?"

# game/remy4.rpy:764
translate frenchtltl _call_skipcheck_29_70eca3a9:

    # Ry "That kind of talk makes it very easy to justify any means when it serves the right cause, whatever that may be. The more important thing is: Have you done anything that you might regret?"
    Ry "Avec ce genre de discours, on peut légitimer n'importe quelle méthode quand elle sert la bonne cause, peu importe ce que c'est. Le plus important est de te demander : As-tu fait quoi que ce soit que tu puisses regretter ?"

# game/remy4.rpy:766
translate frenchtltl _call_skipcheck_29_1a23d3a2:

    # Ry "No matter what you think, keep in mind that you may be held accountable for your actions by others or even yourself."
    Ry "Ce que tu penses importe peu, garde à l'esprit que tu peux être tenu responsable par les autres ou par toi-même pour tes actes."

# game/remy4.rpy:768
translate frenchtltl _call_skipcheck_29_3f85e2cb:

    # Ry "Don't just do something because you think it's for the right reasons. Do it when you are aware of the possible consequences as well."
    Ry "N'agis pas juste parce que tu penses que c'est pour les bonnes raisons. Agis quand tu es également conscient des conséquences éventuelles."

# game/remy4.rpy:770
translate frenchtltl _call_skipcheck_29_0068ec51:

    # c "You're right. I have no way of knowing what my actions here will amount to once I return."
    c "Tu as raison. Je n'ai pas moyen de savoir combien je serai tenu pour responsable de mes actes une fois rentré."

# game/remy4.rpy:772
translate frenchtltl _call_skipcheck_29_3894d46b:

    # Ry "Do you really wish to go back to your own world?"
    Ry "Souhaites-tu réellement rentrer chez toi ?"

# game/remy4.rpy:776
translate frenchtltl _call_skipcheck_29_5bdfb2aa:

    # c "If I didn't, it would only serve to strain the relationship we've been trying to build with your people here, which is something that could be elementary for our survival."
    c "Si je ne le faisais pas, cela ne ferait que détériorer la relation que nous avons essayé de construire avec votre peuple, ce qui est quelque chose qui pourrait être élémentaire pour notre survie."

# game/remy4.rpy:778
translate frenchtltl _call_skipcheck_29_059cb0e7:

    # c "Besides, I didn't do it for those who reign over the city back home, or those who might've wanted to betray me."
    c "De plus, je n'ai pas fait ça pour ceux qui dirigent notre ville, ou pour ceux qui auraient voulu me trahir."

# game/remy4.rpy:780
translate frenchtltl _call_skipcheck_29_89f5f2c3:

    # c "Let's not forget that even under the worst regimes there are just normal people trying to survive, too."
    c "N'oublions pas que, même sous les pires régimes, il y a également des personnes normales qui essayent de survivre."

# game/remy4.rpy:782
translate frenchtltl _call_skipcheck_29_88df99b8:

    # c "If I have the power to make a difference for them, would it be right for me to just leave them be in order to seek my own happiness? That's exactly the kind of thing you were talking about, right?"
    c "Si j'ai les moyens de faire une différence pour eux, serait-il juste de ma part de les laisser tomber dans le but de rechercher mon propre bonheur ? C'est précisément le genre de chose dont tu parlais, n'est-ce pas ?"

# game/remy4.rpy:784
translate frenchtltl _call_skipcheck_29_a6e08e44:

    # Ry "In the end, we all have to make our own decisions."
    Ry "Au final, on doit tous prendre nos propres décisions."

# game/remy4.rpy:786
translate frenchtltl _call_skipcheck_29_9e9f292e:

    # c "If I leave them behind, then I leave them to people like Reza and those who sent me here."
    c "Si je les abandonne, alors je les laisse entre les mains de personnes comme Reza et ceux qui m'ont envoyé ici."

# game/remy4.rpy:788
translate frenchtltl _call_skipcheck_29_fccaee53:

    # Ry "What would you want to do? Overthrow them?"
    Ry "Qu'est-ce que tu voudrais faire ? Les destituer ?"

# game/remy4.rpy:790
translate frenchtltl _call_skipcheck_29_85983446:

    # c "I don't even have the complete picture myself right now. How could I even consider what to do, given what I know?"
    c "Je n'ai même pas une vue d'ensemble. Comment pourrais-je ne serait-ce que réfléchir à quoi faire, étant donné ce que je sais ?"

# game/remy4.rpy:792
translate frenchtltl _call_skipcheck_29_028c20fb:

    # c "What I do know, though, is that if I don't do what they expect of me, there will be consequences."
    c "Ce que je sais, en revanche, c'est que si je ne fais pas ce qu'ils attendent de moi, il y aura des conséquences."

# game/remy4.rpy:794
translate frenchtltl _call_skipcheck_29_a6e40c46:

    # Ry "Maybe you'll just have to wait and see."
    Ry "Il te faudra peut-être attendre de voir."

# game/remy4.rpy:796
translate frenchtltl _call_skipcheck_29_c732224d:

    # c "That's not an option right now. Reza is still out there, armed and dangerous."
    c "Là tout de suite, ce n'est pas une option. Reza est toujours en liberté, armé et dangereux."

# game/remy4.rpy:798
translate frenchtltl _call_skipcheck_29_7935ef31:

    # Ry shy "Don't you think that's a matter better left to our police force? It could be very dangerous for you to interfere."
    Ry shy "Ne penses-tu pas que c'est un problème qu'on ferait mieux de laisser à la Police ? Ca pourrait être dangereux d'interférer."

# game/remy4.rpy:800
translate frenchtltl _call_skipcheck_29_76b58d40:

    # c "It's far too late to reconsider. I've already been tangled up in this too much. Besides, I can't stop now."
    c "C'est trop tard pour renoncer. Je suis déjà beaucoup trop mêlé à tout ça. En plus, je ne peux pas m'arrêter maintenant."

# game/remy4.rpy:802
translate frenchtltl _call_skipcheck_29_98081ebf:

    # c "Don't forget how many he has killed already. He won't hesitate to do so again in order to reach his goal."
    c "N'oublie pas combien de dragons il a déjà tué. Il n'hésitera pas à recommencer pour atteindre son but."

# game/remy4.rpy:804
translate frenchtltl _call_skipcheck_29_0c3095cb:

    # c "In the end, it’s my duty to stop him. If I don’t, how would your people look at humanity?"
    c "Au final, il est de mon devoir de l'arrêter. Si je ne le fais pas, quel regard aura votre peuple sur l'humanité ?"

# game/remy4.rpy:806
translate frenchtltl _call_skipcheck_29_cdb198cc:

    # c "How could I face you again, knowing that my negligence could cause suffering and loss for others here if I just stopped now?"
    c "Comment pourrais-je te regarder en face, en sachant que ma négligence causerait souffrances et pertes pour les gens d'ici si je m'arrêtais maintenant ?"

# game/remy4.rpy:808
translate frenchtltl _call_skipcheck_29_191ba2d2:

    # Ry normal "I wish I could be as brave as you."
    Ry normal "J'aurais aimé être aussi courageux que toi."

# game/remy4.rpy:810
translate frenchtltl _call_skipcheck_29_e1571e2d:

    # c "Don't think bravery has anything to do with it. He must be stopped, one way or another."
    c "Ne t'imagine pas que le courage a quelque chose à voir avec ça. Il doit être arrêté, d'une manière ou d'une autre."

# game/remy4.rpy:812
translate frenchtltl _call_skipcheck_29_7746eb29:

    # Ry "I know. Just be careful, alright?"
    Ry "Je sais. Fais juste attention, d'accord ?"

# game/remy4.rpy:814
translate frenchtltl _call_skipcheck_29_8e19e9e6:

    # c "I'll try."
    c "Je vais essayer."

# game/remy4.rpy:816
translate frenchtltl _call_skipcheck_29_7f7ffe4b:

    # Ry "We've been here for a while now. How about I walk you back to your apartment?"
    Ry "Ca fait un moment que nous sommes là. Et si on retournait à ton appartement ?"

# game/remy4.rpy:818
translate frenchtltl _call_skipcheck_29_f408cad6:

    # c "What about the festival?"
    c "Et pour le festival ?"

# game/remy4.rpy:820
translate frenchtltl _call_skipcheck_29_3183c817:

    # Ry "Well, if you still want to go, we could watch the big fireworks show together in a few days."
    Ry "Eh bien, si tu veux toujours y aller, on pourrait aller voir le grand feu d'artifice ensemble dans quelques jours."

# game/remy4.rpy:822
translate frenchtltl _call_skipcheck_29_e94b288a:

    # c "That's an idea."
    c "C'est une idée."

# game/remy4.rpy:824
translate frenchtltl _call_skipcheck_29_608ebe1f:

    # Ry smile "Alright, let's do that."
    Ry smile "D'accord, faisons comme ça."

# game/remy4.rpy:828
translate frenchtltl _call_skipcheck_29_48b3174b:

    # m "I watched Remy as he got up again, stretching himself in a way that reminded me of a cat before we prepared to leave again."
    m "Je regardai Rémi tandis qu'il se relevait, s'étirant d'une façon qui me rappelait celle d'un chat, avant qu'on ne se prépare à repartir."

# game/remy4.rpy:838
translate frenchtltl _call_skipcheck_29_093348e2:

    # c "Oh, you're still here. How's business?"
    c "Oh, vous êtes toujours là. Comment vont les affaires ?"

# game/remy4.rpy:840
translate frenchtltl _call_skipcheck_29_f25d8745:

    # Fv "As expected. People always get hungry at these events eventually."
    Fv "Comme prévu. Les gens ont toujours un creux lors de ces évènements."

# game/remy4.rpy:842
translate frenchtltl _call_skipcheck_29_0f28374a:

    # Fv "Can I offer you some more meat balls?"
    Fv "Est-ce que je peux vous offrir plus de boulettes de viandes ?"

# game/remy4.rpy:844
translate frenchtltl _call_skipcheck_29_a267b58a:

    # Ry "No, thanks. I'm quite full from my earlier helping."
    Ry "Non merci. J'ai été bien rassasié avec ma part de tout-à-l'heure"

# game/remy4.rpy:846
translate frenchtltl _call_skipcheck_29_39892884:

    # Fv "How about you, [player_name]?"
    Fv "Et pour toi, [player_name] ?"

# game/remy4.rpy:848
translate frenchtltl _call_skipcheck_29_a7a8c23f:

    # c "A tempting offer, but we're just about to call it a day."
    c "Une offre tentante, mais ça ira pour aujourd'hui."

# game/remy4.rpy:850
translate frenchtltl _call_skipcheck_29_652adda2:

    # Fv "I see. Are you going to watch the big fireworks at the end of the festival?"
    Fv "Je vois. Allez-vous assister au grand feu d'artifice à la fin du festival ?"

# game/remy4.rpy:852
translate frenchtltl _call_skipcheck_29_ed13463b:

    # c "That's the plan."
    c "C'est le plan."

# game/remy4.rpy:854
translate frenchtltl _call_skipcheck_29_19ee7d6c:

    # Fv "Be sure not to miss it. You haven’t seen anything until you see the fireworks!"
    Fv "Soyez sûrs de ne pas le rater. Vous n'avez rien vu jusqu'à ce que vous ayez vu le feu d'artifice !"

# game/remy4.rpy:856
translate frenchtltl _call_skipcheck_29_2ac35e6e:

    # c "That's what everyone keeps telling me."
    c "C'est ce que tout le monde n'arrête pas de me dire."

# game/remy4.rpy:858
translate frenchtltl _call_skipcheck_29_147ada33:

    # Ry smile "Because it's true."
    Ry smile "Parce que c'est le cas."

# game/remy4.rpy:862
translate frenchtltl _call_skipcheck_29_b3ef135b:

    # c "Anyway, we should probably get going."
    c "Enfin bref, on devrait peut-être y aller."

# game/remy4.rpy:864
translate frenchtltl _call_skipcheck_29_00d1f219:

    # Fv "Have a good day, then."
    Fv "Passez une bonne journée, alors."

# game/remy4.rpy:866
translate frenchtltl _call_skipcheck_29_b092993f:

    # c "You too."
    c "Vous aussi."

# game/remy4.rpy:874
translate frenchtltl _call_skipcheck_29_cc3f9358:

    # c "And here we are again."
    c "Et nous revoilà."

# game/remy4.rpy:876
translate frenchtltl _call_skipcheck_29_45035126:

    # Ry "Indeed."
    Ry "En effet."

# game/remy4.rpy:878
translate frenchtltl _call_skipcheck_29_51ecee9d:

    # c "Did you want to clean out my bathroom cabinet as well or something?"
    c "Voulais-tu aussi nettoyer mes toilettes ou quoi ?"

# game/remy4.rpy:880
translate frenchtltl _call_skipcheck_29_64cde2e2:

    # Ry smile "That's a good one."
    Ry smile "Bonne blague."

# game/remy4.rpy:884
translate frenchtltl _call_skipcheck_29_d5a4c550:

    # m "He gave a big sigh."
    m "Il lâcha un grand soupir."

# game/remy4.rpy:890
translate frenchtltl remy4skip2_6acc0c28:

    # Ry "You know how I feel. I guess I just wanted to make the most of this day."
    Ry "Tu sais comment je me sens. Je suppose que je voulais profiter à fond de cette journée."

# game/remy4.rpy:894
translate frenchtltl remy4skip2_d210f8ca:

    # c "We're still going to see the fireworks together."
    c "On va toujours voir les feux d'artifices ensemble."

# game/remy4.rpy:896
translate frenchtltl remy4skip2_f123bb35:

    # Ry smile "That's true."
    Ry smile "C'est vrai."

# game/remy4.rpy:898
translate frenchtltl remy4skip2_371a4c0a:

    # Ry normal "You know, I'm very curious to see what your reaction to that will be. It's quite a sight to behold."
    Ry normal "Tu sais, je suis très curieux de voir quelle sera ta réaction à ça. C'est un sacré spectacle à contempler."

# game/remy4.rpy:900
translate frenchtltl remy4skip2_0969ec1b:

    # c "People just keep saying that."
    c "Tout le monde n'arrête pas de dire ça."

# game/remy4.rpy:902
translate frenchtltl remy4skip2_d79351d2:

    # Ry "Maybe you will, too."
    Ry "peut-être que tu finiras par le dire aussi."

# game/remy4.rpy:904
translate frenchtltl remy4skip2_7bf19640:

    # c "We'll see."
    c "Nous verrons."

# game/remy4.rpy:906
translate frenchtltl remy4skip2_8fb23e33:

    # Ry smile "Thanks for the wonderful day, though."
    Ry smile "Merci pour cette journée, c'était génial."

# game/remy4.rpy:908
translate frenchtltl remy4skip2_e085fae4:

    # c "Likewise."
    c "Pareillement."

# game/remy4.rpy:910
translate frenchtltl remy4skip2_6ec78670:

    # Ry normal "Until we meet again."
    Ry normal "A notre prochaine rencontre."

# game/remy4.rpy:912
translate frenchtltl remy4skip2_a4c90328:

    # c "See you."
    c "A plus."

# game/remy4.rpy:942
translate frenchtltl remy4skip2_e7d82850:

    # c "Speaking of which, I've never actually seen you without your tie."
    c "Au passage, je ne t'ai jamais vu sans ta cravate."

# game/remy4.rpy:944
translate frenchtltl remy4skip2_aae91d30:

    # Ry "Well, actually..."
    Ry "Eh bien, en fait..."

# game/remy4.rpy:954
translate frenchtltl remy4skip2_363a934a:

    # Ry "How about now?"
    Ry "Et là ?"

# game/remy4.rpy:956
translate frenchtltl remy4skip2_ffc48ec9:

    # c "You are pretty cute, you know that?"
    c "Tu es plutôt mignon, tu le sais ça ?"

# game/remy4.rpy:958
translate frenchtltl remy4skip2_2ef9b681:

    # Ry normal b "Is that just a term of endearment, or are you actually serious?"
    Ry normal b "Est-ce juste un terme affecteux, ou tu es vraiment sérieux ?"

# game/remy4.rpy:960
translate frenchtltl remy4skip2_3c71a7be:

    # c "I am serious."
    c "Je suis sérieux."

# game/remy4.rpy:962
translate frenchtltl remy4skip2_5cca760f:

    # m "He looked at me, hesitating. Then, he took a step forward, his head slowly moving closer to my own."
    m "Il me regarda, hésitant. Puis, il avança d'un pas, sa tête se rapprochant lentement de la mienne."

# game/remy4.rpy:982
translate frenchtltl remy4skip2_a681ed09:

    # m "When I made the motion to dodge his advance, he stopped in his tracks immediately."
    m "Quand j'eus bougé pour esquiver son avance, il s'arrêta dans son mouvement immédiatement."

# game/remy4.rpy:986
translate frenchtltl remy4skip2_0ed2e312:

    # Ry "I'm sorry, I just thought..."
    Ry "Je suis désolé, je pensais juste que..."

# game/remy4.rpy:988
translate frenchtltl remy4skip2_0746f984:

    # c "Don't worry about it. I'm just not sure if I can do this."
    c "Ne t'en fais pas. Je ne sais juste pas si je suis capable de faire ça."

# game/remy4.rpy:990
translate frenchtltl remy4skip2_8305e336:

    # Ry normal b "Well, it's getting late, so I should probably go now."
    Ry normal b "Bon, il se fait tard, donc je devrais peut-être y aller."

# game/remy4.rpy:992
translate frenchtltl remy4skip2_d42de145:

    # Ry "Do you still want to see the fireworks?"
    Ry "Tu veux toujours aller voir le feu d'artifice ?"

# game/remy4.rpy:994
translate frenchtltl remy4skip2_7f0f33d6:

    # c "Oh, for sure."
    c "Oh, bien sûr."

# game/remy4.rpy:996
translate frenchtltl remy4skip2_a5e30217:

    # Ry "I'll see you, then."
    Ry "A la revoyure, alors."

# game/remy4.rpy:998
translate frenchtltl remy4skip2_ca40710d:

    # c "Until we meet again."
    c "A notre prochaine rencontre."

# game/remy4.rpy:1000
translate frenchtltl remy4skip2_f210ba5f:

    # Ry "Thank you for everything."
    Ry "Merci pour tout."

# game/remy4.rpy:1039
translate frenchtltl remy4skip2_cfd4b66e:

    # m "We met, and my arms enveloped his neck as our lips touched. For a few seconds, we were closer than ever before. During the kiss, he used a lot more tongue than I expected."
    m "Nous nous sommes embrassés, et mes bras enveloppèrent son cou tandis que nos lèvres se touchaient. Pendant quelques secondes, nous fûmes plus proches que jamais. Durant le baiser, il y mit plus de langue que ce à quoi je m'attendais."

# game/remy4.rpy:1043
translate frenchtltl remy4skip2_bea08d55:

    # m "Just after we parted, he finished by giving me a small lick on the cheek."
    m "Juste quand nous nous séparâmes, il me donna un petit coup de langue sur la joue."

# game/remy4.rpy:1047
translate frenchtltl remy4skip2_c08eaffd:

    # Ry "How was that?"
    Ry "Comment c'était ?"

# game/remy4.rpy:1049
translate frenchtltl remy4skip2_f2ac2720:

    # c "Unique, that's for sure."
    c "Unique, ça c'est sûr."

# game/remy4.rpy:1051
translate frenchtltl remy4skip2_dd944d3a:

    # Ry normal b "Maybe I shouldn't wear the tie anymore if this is what happens when I take it off."
    Ry normal b "peut-être que je ne devrais plus porter la cravate si c'est ce qui se passe quand je l'enlève."

# game/remy4.rpy:1053
translate frenchtltl remy4skip2_abf2a97a:

    # c "Actually, I think you should keep it."
    c "En fait, je pense que tu devrais la garder."

# game/remy4.rpy:1055
translate frenchtltl remy4skip2_a0284234:

    # Ry "Really?"
    Ry "Vraiment ?"

# game/remy4.rpy:1057
translate frenchtltl remy4skip2_107876f3:

    # c "Yeah. It looks good on you."
    c "Ouais. Elle te va bien."

# game/remy4.rpy:1059
translate frenchtltl remy4skip2_c47053e6:

    # Ry smile b "Well, if you say so."
    Ry smile b "Bon, si tu le dis."

# game/remy4.rpy:1061
translate frenchtltl remy4skip2_592f9944:

    # Ry normal b "Anyway, it's getting really late, so I should probably get going now."
    Ry normal b "Enfin bref, il se fait tard, donc je ferais mieux d'y aller."

# game/remy4.rpy:1063
translate frenchtltl remy4skip2_7b3e164d:

    # Ry "Don't forget about the fireworks."
    Ry "N'oublie pas pour le feu d'artifice."

# game/remy4.rpy:1065
translate frenchtltl remy4skip2_7f0f33d6_1:

    # c "Oh, for sure."
    c "Oh, bien sûr."

# game/remy4.rpy:1067
translate frenchtltl remy4skip2_a5e30217_1:

    # Ry "I'll see you, then."
    Ry "A la revoyure, alors."

# game/remy4.rpy:1069
translate frenchtltl remy4skip2_ca40710d_1:

    # c "Until we meet again."
    c "A notre prochaine rencontre."

# game/remy4.rpy:1071
translate frenchtltl remy4skip2_f210ba5f_1:

    # Ry "Thank you for everything."
    Ry "Merci pour tout."

translate frenchtltl strings:

    # remy4.rpy:7
    old "Chapter 4 - Remy 4"
    new "Chapitre 4 - Rémi 4"

    # remy4.rpy:11
    old "Chapter 3 - Remy 4"
    new "Chapitre 3 - Rémi 4"

    # remy4.rpy:15
    old "Chapter 2 - Remy 4"
    new "Chapitre 2 - Rémi 4"

    # remy4.rpy:19
    old "Chapter 1 - Remy 4"
    new "Chapitre 1 - Rémi 4"

    # remy4.rpy:227
    old "They're pretty interesting."
    new "Ils sont plutôt intéressants."

    # remy4.rpy:227
    old "I just wasn't in the mood for anything more complex."
    new "Je n'étais pas d'humeur pour quelque chose de plus complexe."

    # remy4.rpy:333
    old "Let's try some together, then."
    new "Essayons-le ensemble, alors."

    # remy4.rpy:333
    old "I'm not a fan either."
    new "Je ne suis pas fan non plus."

    # remy4.rpy:370
    old "An interesting afternoon."
    new "Une intéressante après-midi."

    # remy4.rpy:370
    old "A wonderful friendship."
    new "Une formidable amitié."

    # remy4.rpy:407
    old "It's alright."
    new "Il est correct."

    # remy4.rpy:407
    old "It's bad."
    new "Il est mauvais."

    # remy4.rpy:496
    old "I'll also take the meat balls."
    new "Je prendrai aussi les boulettes de viandes."

    # remy4.rpy:496
    old "How about some veggie skewers?"
    new "Pourquoi pas quelques brochettes végétariennes ?"

    # remy4.rpy:496
    old "Just some grilled cheese for me."
    new "Juste du fromage grillé pour moi."

    # remy4.rpy:972
    old "Look away."
    new "Détourner le regard."

    # remy4.rpy:972
    old "Kiss him."
    new "L'embrasser."

